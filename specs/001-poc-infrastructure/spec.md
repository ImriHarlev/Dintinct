...
3. **Given** the retry policy limit is exhausted for a chunk, **When** the Dispatch & Retry Service determines no more retries are allowed, **Then** a `.hardfail` command file is written to the Proxy outbox for Network B to process.
4. **Given** a `.hardfail` file arrives in Network B, **When** the Assembly Workflow processes it, **Then** the affected file is marked as permanently failed and the final CSV report reflects this status.
...
- **FR-003**: The system MUST define interfaces for all cross-service interactions (IJobRepository, IProxyConfigRepository, IManifestRepository, IJobStateSignalHandler, etc.) in `Shared.Contracts` or the appropriate project.
...
- **FR-008**: The Heavy Processing Activity MUST accept source path info from the workflow, write mock chunk files to the configured Proxy Data Outbox directory, and return a metadata object containing chunk names, total chunk count, and placeholder checksums; actual split/conversion logic MUST be replaced with pseudo-code comments. The number of mock chunks generated MUST be read from a `MockChunkCount` configuration value (environment variable or config file) — no hardcoded chunk count is permitted, as real chunk counts are driven by package size and proxy-imposed size limits and can be arbitrarily large.
- **FR-009**: The Manifest Activity MUST accept the metadata object from the workflow, generate a `manifest.json` file, and write it directly to the configured Proxy Manifest Outbox directory; actual manifest schema construction logic MUST be replaced with a mock that writes a structurally valid JSON file conforming to the agreed schema: `{ "jobId": "…", "originalFile": "…", "totalChunks": 2, "chunks": [{ "name": "chunk1.bin", "index": 1, "checksum": "abc123" }] }`.
- **FR-010**: The Dispatch & Retry Activity MUST handle retry signals from the workflow, re-place the identified chunk into the Proxy Data Outbox, increment the retry count in MongoDB, and generate a `.hardfail` command file when the retry limit is exhausted.
...
- **FR-012**: The Proxy Event Listener (Network B) MUST consume RabbitMQ messages from the configured Proxy queue. Each message body MUST be a JSON object `{ "filePath": "<absolute-or-relative-path-in-Network-B>" }` with Content-Type `application/json`. The listener MUST use `SignalWithStart` to start or signal the Assembly Workflow.
- **FR-013**: The Proxy Event Listener MUST distinguish between `.ERROR.txt`, `.UNSUPPORTED.txt`, valid data chunks, and manifest files by inspecting the `filePath` value from the message envelope, and route each type accordingly to the Assembly Workflow signal.
- **FR-014**: The Assembly Workflow (Network B) MUST use `Workflow.AwaitAsync` to wait for the manifest signal, then wait for the exact number of chunk signals defined in the manifest before triggering assembly.
- **FR-015**: The Assembly Workflow MUST enforce a configurable global timeout, transitioning to `TIMEOUT` status and triggering the Reporting & Dispatch Activity if exceeded.
- **FR-016**: The Manifest & State Activity (Network B) MUST parse the arriving `manifest.json` (schema: `{ "jobId", "originalFile", "totalChunks", "chunks": [{ "name", "index", "checksum" }] }`), persist the expected package blueprint to Network B's MongoDB instance, and return the expected chunk count and per-chunk checksums to the workflow; actual parsing logic MUST be mocked.
- **FR-017**: The Reassembly & Validation Activity MUST be triggered once all chunks have arrived, perform mock concatenation and validation with pseudo-code comments, and return an array of per-file statuses (`COMPLETED`, `FAILED`, `NOT_SUPPORTED`).
...
- **Manifest**: The JSON blueprint written to the Proxy Manifest Outbox. Schema: `{ "jobId": string, "originalFile": string, "totalChunks": number, "chunks": [{ "name": string, "index": number, "checksum": string }] }`. The `checksum` field holds a placeholder value (`"N/A"`) in the POC mock.
...
- Q: What is the `manifest.json` schema that Network A writes (FR-009) and Network B parses (FR-016)? → A: Nested per-chunk object array — `{ "jobId": "…", "originalFile": "…", "totalChunks": 2, "chunks": [{ "name": "chunk1.bin", "index": 1, "checksum": "abc123" }] }`
